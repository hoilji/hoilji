---
layout: page
title: Projects
description: Ongoing and completed research projects
img: assets/img/3.jpg
importance: 3
category: project
---
<div class="col-md-9">
<h4><strong> Ongoing Projects </strong></h4>
</div>

<div class="row align-items-center my-4">
  <!-- Left: Image (1/4) -->
  <div class="col-md-3 text-center">
    {% include figure.html 
      path="assets/img/logo/NRF.png" 
      class="img-fluid rounded"
    %}
  </div>
  <!-- Right: Text (3/4) -->
  <div class="col-md-9">
    <h4>인공지능 활용 PCFC 소재/구조 개발 과제</h4>
    <p>
사업명: 무탄소에너지핵심기술개발>무탄소에너지핵심기술개발<br>
과제명: 인공지능을 활용한 초고전도/초전하 신규소재 기반 PCFC 핵심 요소기술 개발<br>
역할: 총괄 (연구책임: 지호일)<br>
참여기관: 한국에너지기술연구원, 금오공과대학교, 서울대학교, 광주과기원<br>
기간: 2025. 04. 01 ~ 2029. 12. 31 
    </p>
  </div>
</div>

<div class="row align-items-center my-4">
  <!-- Left: Image (1/4) -->
  <div class="col-md-3 text-center">
    {% include figure.html 
      path="assets/img/logo/NST.png" 
      class="img-fluid rounded"
    %}
  </div>
  <!-- Right: Text (3/4) -->
  <div class="col-md-9">
    <h4>수소저장체(LOHC) 직접 활용 저온 PCFC 기술 개발 과제</h4>
    <p>
사업명: 글로벌 TOP 전략연구단<br>
과제명: 차세대 LOHC 수소 추출-활용 융합 혁신 기술<br>
역할: 5세부 (연구책임: 지호일)<br>
총괄연구기관: 한국과학기술연구원 (연구단장: 이관영)<br>
기간: 2024. 09. 01 ~ 2029. 08. 31 
    </p>
  </div>
</div>

<div class="row align-items-center my-4">
  <!-- Left: Image (1/4) -->
  <div class="col-md-3 text-center">
    {% include figure.html 
      path="assets/img/logo/NRF.png" 
      class="img-fluid rounded"
    %}
  </div>
  <!-- Right: Text (3/4) -->
  <div class="col-md-9">
    <h4>인공지능 활용 PCEC 신규 전해질 개발 및 데이터베이스 구축 과제</h4>
    <p>
사업명: 나노·소재기술개발<br>
과제명: 인공지능 및 데이터를 활용한 수전해용 프로톤전도성 소재의 HUB 구축<br>
역할: 공동연구기관 (연구책임: 지호일)<br>
총괄연구기관: 한국에너지기술연구원 (연구책임: 홍종은)<br>
기간: 2024. 07. 01 ~ 2028. 12. 31 
    </p>
  </div>
</div>

<div class="row align-items-center my-4">
  <!-- Left: Image (1/4) -->
  <div class="col-md-3 text-center">
    {% include figure.html 
      path="assets/img/logo/KETEP.png" 
      class="img-fluid rounded"
    %}
  </div>
  <!-- Right: Text (3/4) -->
  <div class="col-md-9">
    <h4>PCEC 대면적화 기술 개발 과제</h4>
    <p>
사업명: 신재생에너지핵심기술개발사업<br>
과제명: 그린수소 생산성 극대화를 위한 중저온 작동형 프로톤 세라믹 수전해 기술 개발<br>
역할: 총괄 (연구책임: 이종호, 실무: 지호일)<br>
참여기관: 한국에너지기술연구원, 홍익대, 명지대, 성균관대, 금오공대<br>
기간: 2022. 04. 01 ~ 2026. 03. 31 
    </p>
  </div>
</div>


<div class="col-md-9">
   <br>
   <br>
<h4><strong> Completed Projects </strong></h4>
</div>


<div class="row align-items-center my-4">
  <!-- Left: Image (1/4) -->
  <div class="col-md-3 text-center">
    {% include figure.html 
      path="assets/img/logo/NRF.png" 
      class="img-fluid rounded"
    %}
  </div>
  <!-- Right: Text (3/4) -->
  <div class="col-md-9">
    <h4>PCEC 소재 및 공정 기술 개발 과제</h4>
    <p>
사업명: 미래수소원천기술개발사업<br>
과제명: 그린수소 대량생산을 위한 고효율 고신뢰성 protonics 기반 수전해전지 원천 소재/공정 기술개발<br>
역할: 주관 (연구책임: 지호일)<br>
참여기관: 금오공과대학교<br>
기간: 2021. 07. 01 ~ 2024. 12. 31 
    </p>
  </div>
</div>

<div class="row align-items-center my-4">
  <!-- Left: Image (1/4) -->
  <div class="col-md-3 text-center">
    {% include figure.html 
      path="assets/img/logo/KIST.png" 
      class="img-fluid rounded"
    %}
  </div>
  <!-- Right: Text (3/4) -->
  <div class="col-md-9">
    <h4>PCEC 기반 전기화학적 암모니아 합성 기술 개발 과제</h4>
    <p>
사업명: KIST Young Fellow<br>
과제명: 열역학적 한계를 극복하는 Protonic Ceramic 기반 암모니아 생산<br>
역할: 단독주관 (연구책임: 지호일)<br>
기간: 2020. 03. 01 ~ 2022. 12. 31 
    </p>
  </div>
</div>

<div class="row align-items-center my-4">
  <!-- Left: Image (1/4) -->
  <div class="col-md-3 text-center">
    {% include figure.html 
      path="assets/img/logo/KETEP.png" 
      class="img-fluid rounded"
    %}
  </div>
  <!-- Right: Text (3/4) -->
  <div class="col-md-9">
    <h4>SOFC 대면적화를 위한 롤캘린더링 공정 기술 개발 과제</h4>
    <p>
사업명: 에너지기술개발사업<br>
과제명: SOFC의 경제성 극대화를 위한 점탄성 전단공정 기반 셀 제조기술 개발<br>
역할: 주관 (연구책임: 지호일)<br>
참여기관: 건국대학교, 한양대학교<br>
기간: 2017. 12. 01 ~ 2020. 11. 30 
    </p>
  </div>
</div>

